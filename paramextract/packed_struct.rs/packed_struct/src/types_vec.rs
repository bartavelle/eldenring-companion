...
use crate::{lib_get_mut_slice, lib_get_slice, PackedStructSlice, PackingError};
    ...
    T: PackedStructSlice,
{
    fn pack_to_slice(&self, output: &mut [u8]) -> Result<(), PackingError> {
        ...
        if output.len() != expected_size {
            return Err(crate::PackingError::BufferSizeMismatch {
    ...
    fn unpack_from_slice(src: &[u8]) -> Result<Self, PackingError> {
        ...
        if item_size == 0 || src.is_empty() {
            return Ok(vec![]);
        }
        let modulo = src.len() % item_size;
        if modulo != 0 {
            return Err(crate::PackingError::BufferModMismatch {
                actual_size: src.len(),
                modulo_required: item_size,
            });
        }
    ...
    fn packed_bytes_size(opt_self: Option<&Self>) -> Result<usize, PackingError> {
        match opt_self {
            ...
            Some(s) => Ok(s.len() * T::packed_bytes_size(None)?),
        }
    }
}
