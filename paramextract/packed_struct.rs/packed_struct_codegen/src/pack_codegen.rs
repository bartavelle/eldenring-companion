...
pub fn derive_pack(parsed: &PackStruct) -> syn::Result<proc_macro2::TokenStream> {
    ...
    {
        let mut reg =
            |src: &dyn quote::ToTokens, target: &dyn quote::ToTokens, field: &FieldRegular| -> syn::Result<()> {
...
fn pack_bits(field: &FieldRegular) -> PackBitsCopy {
        ...
        && field.bit_range_rust.len() >= 8
    {
        ...
        let emit_shift = |s: isize| match s {
            0 => quote! {},
            _ if s > 0 => quote! { << #s },
            _ => {
                let s = -s;
...
fn unpack_field(field: &FieldRegular) -> syn::Result<proc_macro2::TokenStream> {
    ...
    loop {
        match (wrappers.get(i), wrappers.get(i + 1)) {
            (
                Some(SerializationWrapper::Endiannes { ref endian }),
                Some(SerializationWrapper::Integer { ref integer }),
            ) => {
                unpack = quote! {
                    use ::packed_struct::types::*;
                    use ::packed_struct::types::bits::*;
        ...
        if wrappers.is_empty() || i > wrappers.len() - 1 {
            break;
        }
    }
