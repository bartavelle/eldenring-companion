...
fn get_builtin_type_bit_width(p: &syn::PathSegment) -> syn::Result<Option<usize>> {
    match p.ident.to_string().as_str() {
        ...
        "u8" | "i8" => Ok(Some(8)),
        "u16" | "i16" => Ok(Some(16)),
        "u32" | "i32" | "f32" => Ok(Some(32)),
        "u64" | "i64" | "f64" => Ok(Some(64)),
        "ReservedZero" | "ReservedZeroes" | "ReservedOne" | "ReservedOnes" | "Integer" => match p.arguments {
...
fn get_field_mid_positioning(field: &syn::Field) -> syn::Result<FieldMidPositioning> {
    ...
    let field_attributes =
        PackFieldAttribute::parse_all(&parse_sub_attributes(&field.attrs, "packed_field", "packed_struct")?);
...
fn parse_reg_field(
    ...
    if needs_int_wrap {
        ...
        wrappers.push(SerializationWrapper::Integer {
    ...
    if needs_endiannes_wrap {
        ...
        wrappers.push(SerializationWrapper::Endiannes {
...
pub fn parse_num(s: &str) -> Result<usize, String> {
    ...
    if s.starts_with("0x") || s.starts_with("0X") {
        ...
        s.parse()
            .map_err(|e| format!("Invalid decimal number: {:?}, parse error: {:?}", s, e))
    }
}

pub fn parse_struct(ast: &syn::DeriveInput) -> syn::Result<PackStruct> {
    let attributes =
        PackStructAttribute::parse_all(&parse_sub_attributes(&ast.attrs, "packed_struct", "packed_field")?);
            ...
            let mut find_overlaps = |name: String, range: &Range<usize>| {
                for i in range.start..(range.end + 1) {
                    if let Some(Some(n)) = bits.get(i) {
                        return Err(syn::Error::new(
                            name.span(),
                            format!("Overlap in bits between fields {} and {}", n, name),
                        ));
                    }
